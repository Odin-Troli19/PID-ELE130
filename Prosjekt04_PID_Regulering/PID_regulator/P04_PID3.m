...
while ~JoyMainSwitch
    ...
    if k == 1
        ...
        Ki = 0.3;
        Kd = 0.05;
        tau_e = 0.05;   % mer filtrering av e(k)
        I_max = 100;
        I_min = -100;

        % Referanse-verdier og tidspunkt, og indeks for lyd
        ...
        % Filtrering av reguleringsavvik til D-leddet
...
% Save the MATLAB data file
save(filename, ...
    't','r','y','e','e_f','P','I','D','u_A', ...
    'x1','x1_f','x2', ...
    'Kp','Ki','Kd','tau_e','tau_pos','u0','I_max','I_min', ...
    'tidspunkt','RefVerdier');

% Save figure with a matching name
figfilename = strrep(filename,'.mat','.pdf');
LagreMinFigur(fig1, figfilename);

% Optional: also save MATLAB figure file
figmatname = strrep(filename,'.mat','.fig');
savefig(fig1, figmatname);